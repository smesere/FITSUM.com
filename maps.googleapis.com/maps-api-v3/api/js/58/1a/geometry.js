google.maps.__gjsload__('geometry', function(_) {
    var Vra = function(a, b) {
            return Math.abs(_.$i(b - a, -180, 180))
        },
        Wra = function(a, b, c, d, e) {
            if (!d) {
                c = Vra(a.lng(), c) / Vra(a.lng(), b.lng());
                if (!e) return e = Math.sin(_.Zf(a.lat())), e = Math.log((1 + e) / (1 - e)) / 2, b = Math.sin(_.Zf(b.lat())), _.$f(2 * Math.atan(Math.exp(e + c * (Math.log((1 + b) / (1 - b)) / 2 - e))) - Math.PI / 2);
            ...
            b = _.Zf(b.lng());
            c = _.Zf(c);
            return _.$i(_.$f(Math.atan2(Math.sin(e) *
        ...
        Yra = function(a, b) {
            ...
            let d = [0, 0],
                e;
            for (let f = 0, g = _.Si(a); f < g; ++f) e = b ? b(a[f]) : a[f], kA.iD(e[0] - d[0], c), kA.iD(e[1] - d[1], c), d = e;
            return c.join("")
        },
        lA = {
                ...
                for (let k = 0, m = e.getLength(); k < m; ++k) {
                    ...
                    for (let t = 0, u = p.getLength(); t < u; ++t) {
                        ...
                        const z = _.$i(x.lng(), -180, 180),
                            B = Math.max(h, z);
                        h = Math.min(h, z);
                        (B - h > 180 ? c >= B || c < h : c < B && c >= h) && Wra(w, x, c, d, f) < a.lat() && (g = !g)
                    }
                }
                return g || lA.isLocationOnEdge(a, b)
            }
        };
    _.Ga("module$exports$mapsapi$geometry$polyGeometry.PolyGeometry.containsLocation", lA.containsLocation);
    lA.isLocationOnEdge = function(a, b, c) {
        a = _.Jj(a);
        c = c || 1E-9;
        ...
        for (let t = 0, u = g.getLength(); t < u; ++t) {
            ...
            for (let B = 0; B < z; ++B) {
                ...
                if (F - J > 180 ? d + c >= F || d - c <= J : d + c >= J && d - c <= F)
                    if (h = Wra(h, C, d, f, b), Math.abs(h - a.lat()) < c) return !0
            }
        }
        return !1
    };
    _.Ga("module$exports$mapsapi$geometry$polyGeometry.PolyGeometry.isLocationOnEdge", lA.isLocationOnEdge);
    var kA = {
    ...
    _.Ga("module$exports$mapsapi$poly$polylineCodec.PolylineCodec.decodePath", kA.decodePath);
    kA.encodePath = function(a) {
        a instanceof _.Ol && (a = a.getArray());
        a = (0, _.rk)(a);
        return Yra(a, function(b) {
            return [Math.round(b.lat() * 1E5), Math.round(b.lng() * 1E5)]
        })
    };
    _.Ga("module$exports$mapsapi$poly$polylineCodec.PolylineCodec.encodePath", kA.encodePath);
    kA.iD = function(a, b) {
